...
public interface HomeSelectRepository extends CrudRepository<SelectFollowerMovie, String>{
    ...
    @Query("""
         with like_count as ( select response_creater, response_id, sum(view_like) as 'like_count' from view_response tmp1 group by response_creater, response_id ), 
         share_count as ( select count(ORIGIN_ID) as 'share_count', origin_creater as response_creater, origin_id as response_id 
         from response tmp2
         group by origin_creater, origin_id
         union 
         select tmp3.share_count, response_creater, response_id 
         from response 
         inner join (
         select count(*) as 'share_count', origin_creater, origin_id  
         from response tmp2
         group by origin_creater, origin_id
         ) tmp3 using(origin_creater, origin_id)
         ) 
         ...
         , coalesce(like_count.like_count, 0) as 'like', 
         coalesce(share_count.share_count, 0) as 'share' 
         from follow 
         inner join user as user1 on user1.user_id = follow.user_id 
         inner join user as user2 on user2.user_id = follow.followuser_id  
         inner join response on response.response_creater = user2.user_id 
