...
public interface FollowRepository extends CrudRepository<Follow,String>{
    ...
    @Query("with follow_user as ("
         ...
         + "select user.user_name, user.introduction, user.icon , case when follow.followuser_id= :session_user_id then -1 when follow_login_user is null then 0 else 1 end as 'check_follow' from follow "
